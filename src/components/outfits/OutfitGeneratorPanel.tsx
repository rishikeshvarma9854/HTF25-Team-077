...
export default function OutfitGeneratorPanel({ onGenerate, isGenerating }: OutfitGeneratorPanelProps) {
  ...
  const handleGenerate = () => {
    onGenerate({
      ...
      mood: selectedMood,
      weather: useWeather && weatherData ? { temp: weatherData.temperature, condition: weatherData.condition } : undefined,
    });
  };
      ...
      <div>
        ...
        {useWeather && (
          <div className="space-y-4">
            ...
            {/* Weather will be fetched from the Weather Widget above when available. Manual input removed. */}
          </div>
        )}
      </div>
