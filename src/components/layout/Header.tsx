  ...
  return (
    <header className="sticky top-0 z-40 border-b border-brand-200 bg-white/90 backdrop-blur">
      <div className="container-responsive h-16 flex items-center justify-between">
        ...
        {/* Mobile auth controls (visible on small screens) */}
        <div className="flex items-center gap-3 md:hidden">
          {isAuthLoading ? (
            <div className="px-3 py-2 text-sm text-brand-500">Loading…</div>
          ) : authUser ? (
            <Link to="/profile" className="flex items-center gap-2">
              <span className="h-8 w-8 rounded-full bg-white/20 flex items-center justify-center text-xs font-semibold overflow-hidden">
                {profile?.avatar && !avatarError ? (
                  // eslint-disable-next-line jsx-a11y/alt-text
                  <img src={profile.avatar} alt={profile?.name || 'User'} className="h-full w-full object-cover" onError={() => setAvatarError(true)} />
                ) : (
                  <span>{(profile?.name || 'U')[0].toUpperCase()}</span>
                )}
              </span>
            ...
            isSupabaseConfigured ? (
              <Button onClick={signInWithGoogle} variant="outline" aria-label="Sign in">Sign in</Button>
            ) : (
              <button className="inline-flex items-center justify-center rounded-lg px-3 py-2 text-sm font-medium transition-all border-2 border-brand-300 text-brand-900 bg-brand-100" title="Supabase not configured" disabled>
                Sign in
              </button>
            )
          )}
