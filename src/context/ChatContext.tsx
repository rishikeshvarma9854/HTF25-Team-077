...
import { callGemini, hasKeys } from '@/lib/ai';
...
export function ChatProvider({ children }: { children: React.ReactNode }) {
  ...
  const sendMessage = useCallback(
    async (content: string) => {
      ...
      // Try to call Gemini (if keys present) for a richer/stylist reply.
      // Fallback to the local canned generator if LLM is unavailable or fails.
      try {
        if (hasKeys()) {
          const prompt = `You are a helpful fashion stylist assistant. The user asked: "${content}". Wardrobe item count=${context.wardrobeItemCount}, savedOutfitCount=${context.savedOutfitCount}. Keep the reply concise and actionable.`;
        ...
        // Fallback to canned/local response
        ...
        console.warn('AI/Stylist call failed, using local fallback', e);
        const aiResponse = generateAIResponse(content, context);
        const assistantMessage: ChatMessage = {
          id: nanoid(),
          role: 'assistant',
